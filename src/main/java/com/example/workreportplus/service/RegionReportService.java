...
public class RegionReportService implements ReportService {
    ...
    public DailyRegionReportResponse saveReport(RegionReportRequest regionReportRequest) {
        ...
        if (regionReportDto.getRegionId() == null) throw new IllegalArgumentException("Unknown region");
        // Serialize submissions per region; a replacement and all children commit together.
        dsl.selectFrom(com.example.jooq.tables.Region.REGION)
                .where(com.example.jooq.tables.Region.REGION.ID.eq(regionReportDto.getRegionId()))
                .forUpdate().fetchOne();
        ...
        if (groupReports != null) {
            groupReports.forEach(groupReport -> {
                ...
                if (groupIdByName == null || !dsl.fetchExists(dsl.selectOne()
                        .from(com.example.jooq.tables.Group.GROUP)
                        .where(com.example.jooq.tables.Group.GROUP.ID.eq(groupIdByName))
                        .and(com.example.jooq.tables.Group.GROUP.REGION_ID.eq(regionReportDto.getRegionId())))) {
                    throw new IllegalArgumentException("Group does not belong to the selected region");
                }
    ...
    private UUID saveRegionReport(RegionReportDto regionReportDto) {
        ...
        return dsl.transactionResult(conf -> {
            ...
            record.setRegionDescription(Objects.requireNonNullElse(regionReportDto.getRegionDescription(), ""));
            ...
            tx.update(GROUPREPORT).set(GROUPREPORT.STATUS, false)
                    .where(GROUPREPORT.REGION_REPORT_ID.in(tx.select(REGIONREPORT.ID).from(REGIONREPORT)
                            .where(REGIONREPORT.REGION_ID.eq(regionReportDto.getRegionId()))
                            .and(REGIONREPORT.REPORT_DATE.eq(reportDate))
                            .and(REGIONREPORT.ID.ne(savedId))))
                    .execute();
            // Mark prior versions of this region/date inactive.
    ...
    private static UUID[] uuidArray(List<UUID> ids) { return ids == null ? new UUID[0] : ids.toArray(UUID[]::new); }

    private void saveGroupReport(GroupReportDto groupReportDto) {
        ...
        dsl.transaction(configuration -> {
            ...
            record.setFightingContractors(uuidArray(groupReportDto.getFightingContractors()));
            record.setFightingPlaces(uuidArray(groupReportDto.getFightingPlaces()));
            record.setRestContractors(uuidArray(groupReportDto.getRestContractors()));
            record.setRestPlaces(uuidArray(groupReportDto.getRestPlaces()));
            ...
            record.setWorked(groupReportDto.isWorked());
            record.setAmmoVerified(groupReportDto.isAmmoVerified());
