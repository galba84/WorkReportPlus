...
public class WordTemplateService {
    ...
    /** Encode the fully assembled plain-text document once, including monthly reports. */
    public byte[] encodeRtf(byte[] text) throws IOException {
        var document = new javax.swing.text.DefaultStyledDocument();
        try {
            document.insertString(0, new String(text, StandardCharsets.UTF_8), null);
            var output = new java.io.ByteArrayOutputStream();
            new javax.swing.text.rtf.RTFEditorKit().write(output, document, 0, document.getLength());
            return output.toByteArray();
        } catch (javax.swing.text.BadLocationException ex) {
            throw new IOException("Could not encode document", ex);
        }
    }
