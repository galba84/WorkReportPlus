        ...
        try {
            ...
            return raw.entrySet().stream()
                    .collect(Collectors.toMap(
                            Map.Entry::getKey,
                            e -> e.getValue() instanceof List<?> values ? values.stream().map(String::valueOf).collect(Collectors.joining(",")) : Objects.toString(e.getValue(), "")
                    ));
        } catch (IOException e) {
    ...
    public List<GroupReportPrefillDto> getPrefilledGroupReports(UUID regionId, LocalDate date) throws IOException {
        ...
        List<GroupReportPrefillDto> result = groupIds.stream()
                .map(groupId -> {
                    ...
                    List<ContractorDto> restContractors = demoEnabled ? contractorsByGroup : filterByStatus(contractorsByGroup, uuidServiceStatusMap,
